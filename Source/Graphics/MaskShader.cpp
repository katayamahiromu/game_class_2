...
MaskShader::MaskShader(ID3D11Device* device)
{
	...
	//深度ステンシルステート
	{
		...
		HRESULT hr = device->CreateDepthStencilState(&desc, depthStaencilSatate.GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
	}
	...
	//ラスタライザーステート
	{
		...
		HRESULT hr = device->CreateRasterizerState(&desc, rasterizeState.GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
	}
...
void MaskShader::Begin(const RenderContext& rc)
{
	...
	rc.deviceContext->OMSetDepthStencilState(depthStaencilSatate.Get(), 0);
	rc.deviceContext->RSSetState(rasterizeState.Get());
...
void MaskShader::Draw(const RenderContext& rc, const Sprite* sprite)
{
	...
	cbMask.edgThreshold = rc.maskData.edgThreshold;
	cbMask.edgColor = rc.maskData.edgColor;
