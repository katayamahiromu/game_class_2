...
// オーディオソース読み込み
std::unique_ptr<AudioSource> Audio::LoadAudioSource(const char* filename, IXAudio2SubmixVoice* sv)
{
	std::shared_ptr<AudioResource> resource = std::make_shared<AudioResource>(filename);
	return std::make_unique<AudioSource>(xaudio, resource,sv);
}

//サブミックスボイスの作成
std::unique_ptr<SubMixVoice> Audio::MakeSubMix()
{
